...
#자전거 대여량 범주화
for(i in 0:4){ # bike 대여 count에 따라 범주화(200단위씩 끊어서)
  df$CatCount[df$count >= i*200 & df$count < (i+1)*200] <- (i)
}
rm(i) #불필요한 변수 삭제
  ...
  for (i in 1:4){ # of weather
    for(j in 1:24){ # of hour
      for(k in 1:12){ # of months
        ...
        frq5<-c(frq5,sum(df$dmfcc[df$weekday==h & df$weather==i & df$hour==j & df$months==k & df$CatCount==4]))
      }
    }
  }
}
...
for(j in 1:24){ # of hour
  ...
  frq5<-c(frq5,sum(df$dmfcc[df$hour==j & df$CatCount==4]))
}
table.hour<-data.frame(hour,frq1,frq2,frq3,frq4,frq5)
